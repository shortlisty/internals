...
OiQb closes the brief-to-approval loop in two steps.

**Step 1 — Build the library.** The agency connects their existing files (Drive, Notion, email) and uploads venue documents. OiQb extracts the structured data — capacity, catering policy, AV specs, restrictions, contacts — and builds a searchable venue catalog. A human verification step keeps the data trustworthy.
...
**Senior event manager** — uses OiQb on every brief. Searches the catalog, generates the pitch, monitors client interaction. The tool replaces their inbox and their spreadsheet.
...
Every pitch board sent to a client is a passive product demo. The "Powered by OiQb" footer on the free tier reaches event buyers who then ask their own agencies "what is this tool you used?" One paying agency can introduce the product to ten potential clients per month.
...
**Docs:** [What is OiQb?](../../README.md) · [Product Structure](product.md) · [Market Structure](../market.md) · [Architecture](../../platform/architecture.md) · [Vision](../../roadmap/vision.md)
