...
The Master Catalog is populated via three channels — platform seed data (Liquibase XML for 50–200 tier-2 city venues at launch), admin CRUD imports (internal team), and provider web scrapers such as the `venueintelligence-mc-ingest-tagvenue-scraper` writing JSON consumed by `venueintelligence-master-venue-loader`. Tenant data never flows back into the Master Catalog to preserve data isolation and client confidentiality.
