...
5. Record the raw structured data as a **`MasterVenueRecord` JSON** entry — same field shape that `venueintelligence-mc-ingest-tagvenue-scraper` emits, so these manual entries are interchangeable with scraper entries and feed directly into the same `venueintelligence-master-venue-loader` pipeline later.
6. Submit the JSON through either the **admin MasterVenue CRUD API** or the `venueintelligence-master-venue-loader --file` CLI so it is inserted into `public.master_venue` with an associated `master_venue_external` row (`external_source='platform_seed'`). This is a live test of the importer UPSERT dedup pipeline.
...
This process is simultaneously: (a) the 50-document extraction accuracy benchmark from the vision strategic bets, (b) the `venueintelligence-master-venue-loader` fuzzy dedup & merge integration test, (c) the concierge onboarding demo dataset for every future concierge pilot.
