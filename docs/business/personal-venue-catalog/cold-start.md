...
5. Record the raw structured data as a **`MasterVenueRecord` JSON** entry — same field shape that `shortlisty-mc-ingest-tagvenue-scraper` emits, so these manual entries are interchangeable with scraper entries and feed directly into the same `shortlisty-master-venue-loader` pipeline later.
