...
> **Purpose:** Strategic reference for the competitive positioning of OiQb across the event tool ecosystem. Used as context for the more focused DSR competitive analysis in [Digital_Sales_Room_for_Events/comparison.md](../../Digital_Sales_Room_for_Events/comparison.md).
...
| Tool                 | Website                                                                                | Strength                                                                                                                          | Overlap with OiQb                                                                                                |
| -------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
...
**Gaps relevant to OiQb Intelligence:**
...
**Verdict:** Not a direct competitor. Cvent is a venue marketplace. OiQb Intelligence is an intelligence layer for your own venue portfolio. They could be _complementary_ (import discovered venues from Cvent into OiQb).
...
**Verdict:** Different side of the market. Tripleseat serves venues; OiQb serves planners. The intelligence architectures are fundamentally different: Tripleseat mines structured operational data; OiQb mines unstructured documents.
...
**Verdict:** Enterprise venue ops software. No overlap with OiQb's document intelligence core.
...
**Verdict:** Facilitron is a public-sector facility operations and community rental platform. It serves the venue operator side — helping schools and cities manage their own spaces and earn rental revenue from the community. It has no overlap with OiQb's core use case. A school district using Facilitron to manage gym rentals is not an OiQb customer; an event agency trying to find and brief that school's gym is.
...
**Verdict:** Propared is production operations software for the teams _producing_ an event — stage managers, production managers, technical directors. OiQb Intelligence is knowledge management software for event managers _finding and briefing_ venues. The workflows are adjacent but the users and the data are entirely different. A production manager at a theatre company uses Propared to schedule the load-in; the event agency that booked that theatre used OiQb to find it.
...
**Verdict:** Irrelevant to OiQb's positioning. Different price/feature tier entirely. Planning Pod (see above) is the more capable tool in this tier and has its own entry.
...
**Verdict:** Same sourcing/discovery quadrant as Cvent and VenueScanner, but lighter-weight and SMB-accessible. Ventur3 helps planners find and brief venues they haven't worked with — OiQb manages the portfolio of venues they already know and trust. Complementary: a planner discovers a venue through Ventur3's RFP process, then ingests that venue's response documents into OiQb to build a permanent, searchable profile.
...
**Verdict:** VenueScanner is a consumer-grade venue search engine, not a planner intelligence tool. A planner who already knows their preferred venues gets nothing from VenueScanner — it only helps with first-pass discovery of venues they haven't worked with yet. Complementary to OiQb, not competitive.
...
**Verdict:** Same quadrant as VenueScanner and Cvent — a venue marketplace/sourcing tool for _discovering_ new venues. OiQb solves the adjacent and complementary problem: once you've found and worked with venues, how do you manage, extract intelligence from, and search across everything you already know about them. A planner could use VenueFindAI to discover a venue, then use OiQb to ingest that venue's documents and build a permanent, searchable profile.
...
**Verdict:** Niche venue operations software for a specific vertical (performing arts). No overlap with OiQb's core use case. Closer to a lighter-weight Momentus than anything in OiQb's competitive set.
...
**Verdict:** Planning Pod is Tripleseat's most direct SMB/mid-market competitor — both serve venue operators, not planners. Planning Pod wins on pricing accessibility and modularity where Tripleseat wins on scale and depth of transactional data. Neither has anything to do with OiQb's use case. The floor plan designer is the only surface-level overlap: Planning Pod lets a venue build and reuse floor plan layouts for BEOs; OiQb parses floor plan documents uploaded by planners and extracts structured intelligence from them. Entirely different users, entirely different data flows.

---

### 1.2 Digital Asset Management (DAM) Platforms

DAM platforms are the closest adjacent category to OiQb Intelligence — they centralize, tag, search, and distribute digital files for marketing and brand teams. Unlike the booking/CRM tools above, DAMs do handle unstructured files (images, PDFs, videos). But their intelligence is built for brand governance, not venue intelligence.
...
**The OiQb displacement:** Google Drive is where files live. OiQb is where venue knowledge lives. OiQb does not replace Drive — it reads the files in Drive and makes their contents searchable.
...
**The OiQb displacement:** Notion is what planners build when they want structure but can't afford the time to maintain it. OiQb fills the same structural role — venue profiles with fields — but populated automatically from source documents rather than by hand.
...
**The OiQb displacement:** Airtable is the most sophisticated DIY alternative. Planners who use Airtable understand the value of structured venue data — they just shouldn't have to maintain the structure themselves. OiQb is Airtable for venue intelligence, with the data entry handled by AI.
...
**The OiQb displacement:** The spreadsheet is the symptom OiQb is treating. "Every planner has one. Nobody trusts it." — already in the homepage copy for good reason.
...
**The OiQb displacement:** OiQb is the extraction layer that should sit between "venue sends a PDF by email" and "team can search that PDF's contents." The inbox is the source; OiQb is the destination.
...
**The OiQb displacement:** ChatGPT answers a question about one document right now. OiQb remembers every document ever uploaded, extracts structured data the whole team can search, and flags when two documents about the same venue disagree. The difference between a conversation and a knowledge base.

---

### 1.5 Document Intelligence & ETL Platforms

These are the infrastructure players. They are the technical substrate that OiQb's pipeline either competes with or can leverage.
...
**Relevance to OiQb Intelligence:**

- Unstructured.io is what OiQb's ETL layer _could use as a backend_ rather than building from scratch
...
**Strategic insight:** OiQb doesn't need to reinvent document parsing. Unstructured.io (or Docling) handles the extraction layer. OiQb's value add is the _venue-specific intelligence_ on top — the domain schema, the aggregation model, the search experience, the team collaboration.
...
**Why it matters for OiQb Intelligence:**
...
**Strategic decision for OiQb Intelligence:** Use Docling as the primary document parsing layer. It handles the structural extraction (layout, tables, text) and Spring AI's ETL pipeline then handles chunking, embedding, and vector storage.
...
**Why it's the right choice for OiQb Intelligence:**
...
**Relevance to OiQb Intelligence:** These are Python-ecosystem tools. Since OiQb Intelligence is Java/Spring Boot, they are not directly applicable. Spring AI is the Java equivalent and has caught up rapidly.

**Note:** If OiQb ever needs a Python microservice for specialized extraction (e.g., advanced floor plan analysis), LlamaIndex's LlamaParse is best-in-class for complex PDFs.
...
**Pattern:** All these companies are _horizontal_ document intelligence APIs. OiQb's opportunity is to be _vertical_ — deeply specialized for venue documents (floor plans, venue decks, CAD files, spec sheets). Horizontal tools extract generic fields. OiQb extracts venue-specific intelligence with a purpose-built schema.

---

### 1.6 Floor Plan Intelligence & Spatial Data Tools

A distinct cluster of tools that deal with floor plan geometry, spatial extraction, or indoor location infrastructure. None of them are venue management or event planning platforms, but they are directly adjacent to OiQb's document intelligence layer — specifically the CAD/floor plan ingestion pipeline. Understanding where they sit clarifies what OiQb needs to build versus what it can leverage or ignore.
...
**Verdict:** SeatPlan.io solves the downstream problem — arranging guests in a space the planner has already chosen. OiQb solves the upstream problem — knowing and comparing the spaces themselves. The AI floor plan import is the only genuine overlap: both tools ingest floor plan PDFs. But SeatPlan produces a seating layout; OiQb extracts structured venue intelligence. Complementary: a planner uses OiQb to decide which room to book, then SeatPlan.io to seat the guests.
...
**Verdict:** FloorScan.ai is a construction industry tool whose core capability — AI geometry extraction from PDF floor plans — is the same technical problem OiQb faces for venue CAD files. It's not a competitor but a technical reference point: the accuracy benchmark (sub-30s analysis, confidence-scored element detection, DXF export) is the quality bar OiQb's floor plan parsing pipeline should target. OiQb's differentiation is what happens _after_ geometric extraction: mapping room dimensions to event configurations, seating capacities, and a searchable venue intelligence schema.
...
- FloorPlan API is a potential _infrastructure component_ for OiQb's floor plan pipeline — the same role Apache Tika plays for text extraction
...
**Strategic decision for OiQb Intelligence:** FloorPlan API (or an equivalent: CubiCasa, Archilogic, or an in-house Docling-based pipeline) handles geometric extraction from raster floor plans. OiQb's proprietary layer is the venue intelligence schema mapped _onto_ that geometry — room identifiers, configuration capacities, sightlines, access points — none of which FloorPlan API provides.

**Verdict:** Not a competitor. A candidate infrastructure vendor for the geometric extraction step of OiQb's CAD/floor plan ingestion pipeline.
...
**Why it appears here:** The IMDF structured indoor map format is relevant to OiQb's long-term architecture. IMDF is the OGC standard for portable venue maps — georeferenced vector geometry with named rooms, levels, and zones. If OiQb ever moves beyond document extraction into interactive floor plan rendering (Phase 3+), IMDF is the output format to target. Open Location Stack's Floor Plan Editor demonstrates what a browser-based IMDF authoring tool looks like.

**Verdict:** No competitive overlap. Open Location Stack targets logistics and manufacturing RTLS deployments. OiQb targets event planning knowledge management. The IMDF format is worth monitoring as a potential output standard for OiQb's structured venue map layer.

---

### 1.7 Competitive Gap Summary

| Capability                               | Cvent            | Tripleseat      | Momentus        | Facilitron         | Propared         | Planning Pod    | VenueScanner    | VenueFindAI     | Ventur3             | Planner Hero         | VenueArc        | Spark (GEVME/PCMA)    | Bynder            | Brandfolder         | Unstructured.io | G Drive / Dropbox | Notion / Airtable | Spreadsheets | Email (inbox) | ChatGPT / Claude    | SeatPlan.io           | FloorScan.ai      | FloorPlan API      | Open Location Stack | OiQb           |
| ---------------------------------------- | ---------------- | --------------- | --------------- | ------------------ | ---------------- | --------------- | --------------- | --------------- | ------------------- | -------------------- | --------------- | --------------------- | ----------------- | ------------------- | --------------- | ----------------- | ----------------- | ------------ | ------------- | ------------------- | --------------------- | ----------------- | ------------------ | ------------------- | ------------------- |
...
**Docs:** [What is OiQb?](../../README.md) · [Business Proposal](proposal.md) · [Competitive Landscape](comparison.md) · [Intelligence Layer](../../platform/intelligence.md) · [Architecture](../../platform/architecture.md)
