...
> **Purpose:** Explain what OiQb Intelligence is made of in business terms — the tenant app, the four capability pillars, the UI concept, and the positioning logic. Read this before the architecture reference.

---

## What the product is

OiQb Intelligence is a knowledge base and workflow tool for event managers and event agencies. Its job is to help them organise venues faster, find the right one sooner, and run better events — without the manual work that currently slows everything down.
...
"Product" in PIM maps to "venue" in OiQb. Each venue is a structured record with a schema. The PIM layer enforces that schema, tracks provenance (which source each field came from), manages confidence scores, and resolves conflicts when multiple documents disagree.
...
DAM in OiQb Intelligence is not generic asset storage. Assets are attached to venues and to specific rooms or spaces within a venue. A floor plan is not just a file — it is a spatial asset linked to a room with known dimensions. That linkage is what makes the asset searchable and useful, not just stored.
...
OiQb occupies a gap that existing tools do not fill.
...
OiQb combines ETL, PIM, DAM, and search in a single tenant app focused specifically on the venue management workflow. It is not a general-purpose tool adapted to events — it is built from the ground up for event professionals, at the interface density that working professionals need.

---

**Docs:** [What is OiQb?](../../README.md) · [Business Proposal](proposal.md) · [Competitive Landscape](comparison.md) · [Architecture](../../platform/architecture.md) · [Vision](../../roadmap/vision.md)
