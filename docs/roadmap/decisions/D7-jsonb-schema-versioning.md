...
- `VenueMetadataMigrator` singleton in `venueintelligence-model` (shared library, zero Spring deps).
...
- **The migrator is the same singleton on read and write, so drift is impossible.** Because `VenueMetadataMigrator` lives in `venueintelligence-model` and is imported by both `venueintelligence-catalog-service` (reads/writes) and `venueintelligence-catalog-processing-worker` (writes), both services use bit-identical migration chain logic. There is no path where a writer stamps V5 via a different migration sequence than a reader's in-memory V5 output produces. Classpath-identical migrator instance guarantees writer→reader agreement regardless of which service touches the row.
