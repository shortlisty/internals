...
- Service code review rule: any class in `venue-service` or `venue-ingestion-worker` that contains more than `@Bean` definitions, `@RabbitListener` annotations, and HTTP DTO translation must be challenged. If the class contains an `if (venue.getCatering() == ...)` business logic line, it belongs in `mi-venue-model`, not the service.
