...
The BENE ingestion pipeline reads venue documents (PDF decks, floor plans, spec sheets, CAD files) and extracts structured metadata (capacity, catering policy, restrictions, etc.). Two production-grade document readers are available in the Java/self-hosted ecosystem:
