...
> **Purpose:** Describe where StashRoom Intelligence is going and why — not what has been built. For completed work, see [CHANGELOG.md](../../CHANGELOG.md) and the milestone files.

---

## One sentence

StashRoom is the collaborative workspace where event agencies turn their venue knowledge into a signed-off proposal — so a client brief becomes a confirmed venue spec in hours, not days.
...
A client brief arrives. The planner opens StashRoom, filters the venue library in under a minute, picks three options, and sends the client a link. The client opens an interactive board, browses floor plans and photos, adjusts the configuration, and clicks Approve. The planner receives a notification. Both sides have a timestamped, immutable record of exactly what was agreed. The whole cycle takes hours, not days.
...
StashRoom becomes the layer that closes the gap between "we know which venues work for this client" and "the client has confirmed and we can proceed." The venue catalog feeds the pitch. The pitch generates the approval. The approval locks the spec. Each step is tracked, sourced, and auditable.

---

## Product structure

StashRoom has two interlocking layers.
...
An agency that can respond to a brief with a polished, interactive shortlist in under an hour wins deals that a slower competitor loses. StashRoom's value is measured in hours saved and deals won, not in database entries.
...
- **Booking and payments.** Contracts, invoicing, deposit collection, and calendar holds are venue operator problems. StashRoom ends at sign-off on the spec, not at the financial transaction.
- **Venue discovery and marketplace.** StashRoom works with venues the agency already knows. Finding new venues is a separate workflow — and a Phase 3 direction at earliest.
- **CRM and deal pipeline.** StashRoom is not a sales CRM. It does not manage leads, stages, or follow-up sequences.
- **Event execution tools.** Timelines, crew management, run-of-show, and on-site logistics are out of scope. StashRoom covers the venue selection and sign-off phase only.
...
An approved board means the product completed its job: the agency got sign-off, the client has a confirmed spec, and StashRoom was the instrument that made it happen. Venues ingested and searches run are inputs. Approvals are the outcome.
...
**Outward:** a two-sided layer where venues pay to maintain a verified public profile that agencies can pull into pitches directly, without manual ingestion. That turns StashRoom into a marketplace — but only after the agency-side workflow is embedded and trusted. The venue side is years away. Nothing in near-term planning is designed to serve venues.

---

**Docs:** [What is StashRoom?](../README.md) · [Personal Venue Catalog](../business/Personal_Venue_Catalog/product.md) · [Digital Sales Room](../business/Digital_Sales_Room_for_Events/README.md) · [Market Structure](../business/market.md) · [Architecture](../platform/architecture.md)
