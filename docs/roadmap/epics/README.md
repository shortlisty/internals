...
| ------------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Provenance & confidence badge pitch renderer      | C (DSR)        | Deal Room client-facing trust model requires provenance badges and 3-tier confidence dots on every venue card, visible to both planner and client per D16.                                                                                                              | E8 at Done, D16 ADR accepted                         |
| Colored label system + legend builder             | C (DSR)        | Planner-configurable color legend (Recommended / Good fit / Caveats / Backup) + per-venue label selection inside Deal Room; legend is client-visible and editable per pitch.                                                                                            | E8 In progress, D16 ADR accepted                     |
| Dynamic context-aware select boxes in Deal Room   | C (DSR)        | Planner-configures contextual questions per pitch ("Which date?" "Preferred format?" "AV required?") whose options are drawn only from the proposal's actual venues and event context. Client responses are structured events, not free text.                           | E8 In progress, D16 ADR accepted                     |
| Deal Room Timeline / immutable history viewer     | C (DSR)        | Plain-English chronological activity feed (both sides see the same log) per D16 append-only event log. Plain sentences only — no JSON dumps, no database ids.                                                                                                           | E8 at Done, D16 ADR accepted                         |
| Mood-board / vision-card context for chat-search  | B (PVC)        | Planner or client drops 20–30 mood-board reference images (style, lighting, vibe) into a pitch context card; chat-search and venue recommendations use visual-similarity + keyword signals to surface venues matching the vibe. Inspired by Planning Pod Vision Boards. | D15 extraction at In progress, image-embedding model |
| Aisle Planner sync (proposal + timeline + assets) | D (Post-v1.0)  | Tier-2 integration: pull brief questionnaire answers + style tags; push approved venue as proposal seed, timeline anchor milestone, and design-board asset library sync.                                                                                                | E9 promoted, Tier-1 integrations at Done             |
| Planning Pod sync (BEO seed + floor-plan bi-sync) | D (Post-v1.0)  | Tier-3 integration: pull questionnaires/briefs/budgets; push approved venue as BEO seed, budget line items; bi-directional PDF floor-plan asset sync between Shortlisty catalog and Planning Pod layout tool.                                                           | E9 promoted, Tier-2 at In progress                   |
| Tier-1 storage integrations                       | D (Post-v1.0)  | Google Drive + Dropbox OAuth connect → one-click folder import directly into extraction pipeline. Replaces concierge-model manual download-upload.                                                                                                                      | E9 integrations epic promoted to real E{N}           |
...
Do not let this table grow beyond 10 rows. Review it monthly: promote candidates to real epics, or delete them with a one-line reason recorded in [CHANGELOG.md](../../../CHANGELOG.md). Removed in this round: `Tier-2 CRM/workflow integrations` split into Aisle Planner sync as its own candidate; `Tier-3 project tool integrations` split into Planning Pod sync as its own candidate; `Multi-stakeholder voting` deferred — revisit only after DSR multi-approver pattern stabilises.
