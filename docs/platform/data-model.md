...
```
PHOTO          — image file (jpg, png, webp, heic)
               → requires photo_category
               → thumbnail generated after upload

FLOOR_PLAN     — architectural drawing or spatial layout (pdf, png, jpg, dwg)
               → no photo_category
               → extraction: spatial text, room labels

PDF_DECK       — general venue presentation, brochure, or spec deck (pdf)
               → extraction: full text → metadata fields

SPEC_SHEET     — single-page technical spec (pdf, docx)
               → extraction: structured field extraction (capacity, AV, etc.)

MENU           — catering / F&B menu (pdf, docx)
               → extraction: catering fields enrichment

PRICE_LIST     — pricing table or rate card (pdf, csv, xlsx)
               → extraction: pricing fields enrichment
               → if csv/xlsx: table_data populated

DATA_TABLE     — any structured tabular data (csv, xlsx)
               → table_data populated
               → no text extraction

VIDEO          — venue tour or event footage (mp4, mov)
               → thumbnail (cover frame) generated
               → no text extraction

CAD_FILE       — AutoCAD or vector drawing (dwg, dxf, svg)
               → no extraction
...
#### Photo category taxonomy

Applies only when `asset_type = PHOTO`. Controls gallery grouping in UI.

```
EXTERIOR       — facade, entrance, surroundings
INTERIOR       — main hall, rooms, corridors
SETUP          — room configured for an event (banquet, theater, etc.)
DETAIL         — architectural or decor detail
CATERING       — food presentation, bar, kitchen
OUTDOOR        — terrace, garden, rooftop
TEAM           — staff photos (optional, rarely used)
OTHER          — uncategorised photo
```

#### §2c Table data schema (`table_data` JSONB)

Used for `DATA_TABLE`, `PRICE_LIST` when source is CSV or XLSX. Stores parsed tabular content
for direct UI rendering without re-fetching the original file.
...
Constraints: max 500 rows, max 20 columns. Larger files are stored as-is in S3 and not
parsed into `table_data` — `extraction_status` is set to `COMPLETED` with a `size_exceeded`
flag in `extracted_text`.
...
Defined in `venueintelligence-process`. Owned by `venueintelligence-catalog-processing-worker`. See [services.md](services.md) §4c.
...
| Bump ownership      | `venueintelligence-model` library — only the shared model may define `CURRENT_SCHEMA_VERSION`                |
...
Lives in `venueintelligence-model` (shared library, zero Spring deps — pure Java). Both services use the **same** migrator instance.
...
Each `N → N+1` step is a single-responsibility class implementing `MetadataMigration` interface (defined in `venueintelligence-process`). Migrations are registered in an ordered list inside `VenueMetadataMigrator` — the order is the source of truth.

```java
package com.iqkv.venueintelligence.venue.model;

// venueintelligence-model: no Spring, no framework deps
...
```xml
<resultMap id="VenueResultMap" type="com.iqkv.venueintelligence.model.venue.Venue">
  <id     property="id"        column="id"/>
  <result property="name"      column="name"/>
  <result property="metadata"  column="metadata"
          typeHandler="com.iqkv.venueintelligence.model.metadata.VenueMetadataTypeHandler"/>
</resultMap>
```
...
Migrations live in `venueintelligence-model` under `src/main/resources/db/changelog/tenant/`. Both services include the library on their classpath; `venueintelligence-catalog-service` runs the migrations on startup (same pattern as IAM).
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------- |
| `venues`                | `id` UUID PK, `name` VARCHAR(255), `display_name` VARCHAR(255), `city` VARCHAR(100), `country_code` CHAR(2), `website_url` VARCHAR(500), `primary_photo_asset_id` UUID, `status` VARCHAR(20), `profile_stage` VARCHAR(20), `source` VARCHAR(20), `metadata` JSONB, `description_embedding` VECTOR(1536), `location` GEOGRAPHY, `master_venue_id` UUID nullable, `last_used_in_sales_room_at` TIMESTAMP | `venueintelligence-catalog-service`           |
| `venue_annotations`     | `id` UUID PK, `venue_id` UUID FK, `annotation_type` VARCHAR(20), `text_value` TEXT, `color_hex` VARCHAR(7), `numeric_value` NUMERIC(3,1), `is_private` BOOLEAN, `created_by` UUID                                                                                                                                                                                                                      | `venueintelligence-catalog-service`           |
| `venue_assets`          | `id` UUID PK, `venue_id` UUID FK, `asset_type` VARCHAR(50), `photo_category` VARCHAR(30), `display_order` SMALLINT, `label` VARCHAR(255), `cdn_url` TEXT, `thumbnail_cdn_url` TEXT, `table_data` JSONB, `extraction_status` VARCHAR(20), `extracted_text_embedding` VECTOR(1536), `uploaded_at` TIMESTAMP, `updated_at` TIMESTAMP                                                                      | `venueintelligence-catalog-service`           |
| `extraction_jobs`       | `id` UUID PK, `asset_id` UUID FK, `status` VARCHAR(20), `extractor_type` VARCHAR(50), `extracted_data` JSONB, `confidence_scores` JSONB                                                                                                                                                                                                                                                                | `venueintelligence-catalog-processing-worker` |
| `venue_metadata_events` | `id` UUID PK, `venue_id` UUID FK, `event_type` VARCHAR(50), `event_data` JSONB — append-only                                                                                                                                                                                                                                                                                                           | `venueintelligence-catalog-service`           |
| `item_vectors`          | `id` UUID PK, `content` TEXT, `metadata` JSONB, `embedding` VECTOR(1536) — Spring AI PgVectorStore table. Defined in `venueintelligence-process` changelog.                                                                                                                                                                                                                                            | `venueintelligence-catalog-processing-worker` |
| `ai_cost_tracking`      | `id` UUID PK, `provider` VARCHAR(50), `model` VARCHAR(100), `tokens_used` INTEGER, `cost_usd` NUMERIC(10,6)                                                                                                                                                                                                                                                                                            | `venueintelligence-catalog-processing-worker` |
...
4. **Role-level hardening (pre-production):** The `venueintelligence-catalog-service` connection pool holds `SELECT` on `public.master_venue` / aliases / external. `INSERT / UPDATE / DELETE` on `public` tables is revoked; only the `master_catalog_admin` connection holds write grants.
