...
| [services.md](services.md)                           | §4 Service Architecture · §4a `mi-venue-model` · §4b S3 Storage · §4c `mi-data-intelligence`      | Service decomposition, table ownership, shared library internals, S3 key layout, lifecycle rules, GDPR deletion cascade                                  |
...
```
mi-data-intelligence  (platform JAR — no vertical deps)
        │
        ▼
mi-venue-model        (venue-domain JAR — imports mi-data-intelligence)
        │
...
| Which table `mi-venue-processing-worker` may not write to                          | [services.md § Table Ownership](services.md)                    |
