...
> **Audience:** Engineers.
> **Purpose:** The full async document processing pipeline inside `shortlisty-catalog-processing-worker` — parse, transform, load stages, asset type routing, and processing SLAs.

---

## Related Documents

- [services.md](services.md) — service responsibilities, table ownership, `shortlisty-data-intelligence` library
...
## 5. ETL Pipeline (`shortlisty-catalog-processing-worker`)
...
The pipeline contracts — `ExtractionJob`, `ExtractionStatus`, `ExtractorType`, `AssetType`, event POJOs (`AssetUploadedEvent`, `ExtractionCompletedEvent`, `ExtractionFailedEvent`) — are defined in `shortlisty-data-intelligence` (see [services.md § 4c](services.md)). The venue-specific enricher (`VenueMetadataEnricher`) and its output type (`VenueMetadata`) come from `shortlisty-venue-model` (see [services.md § 4a](services.md)).
...
3. **Extract** — `VenueMetadataEnricher` (custom `DocumentTransformer`, venue-specific, from `shortlisty-venue-model`): calls GPT-4o with structured output schema matching the venue canonical field set (see [data-model.md § Canonical Field Set](data-model.md)), returns `VenueMetadata` POJO with confidence scores per field. The enricher is the **only venue-specific component** in the pipeline — all surrounding plumbing is generic.

---

### Stage 3 — Load

1. **Embed** — `EmbeddingModel` (`text-embedding-3-small`, 1536 dims). Generic — from `shortlisty-data-intelligence`. Skipped for `DATA_TABLE` and `PRICE_LIST` (CSV/XLSX) — `table_data` is already populated; only metadata stored in `item_vectors`.
2. **Store** — `TenantAwarePgVectorStore` writes chunks + embeddings to `item_vectors` table in the tenant's schema. Table defined in `shortlisty-data-intelligence` changelog (see [services.md § 4c](services.md)).
...
4. **Aggregate** — publishes `ExtractionCompletedEvent` (from `shortlisty-data-intelligence`) → `MetadataAggregationConsumer` in `shortlisty-catalog-service` updates `venues.metadata` via `VenueMetadataMigrator.ensureCurrent()` (from `shortlisty-venue-model`) and recalculates `profile_stage` (`SEEDED → ENRICHED → CURATED → READY`) in the same DB transaction. Full aggregation design including `profile_stage` transition rules in [aggregation.md](aggregation.md).
...
Retry on failure: 3 attempts with exponential backoff. After 3 failures → `ExtractionFailedEvent` (from `shortlisty-data-intelligence`) → user notification.
...
public class AssetExtractionConsumer {
    ...
    private final VenueMetadataEnricher enricher;   // from shortlisty-venue-model
    private final TableDataParser tableDataParser;  // from shortlisty-data-intelligence
